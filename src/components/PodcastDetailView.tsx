...
const PodcastDetailView = () => {
  ...
  const currentPodcast: PodcastInfo | undefined = podcastList?.find(podcast => podcast.id == id)
  ...
  return (
    <div>
      {currentPodcast && <PodcastCard id={currentPodcast?.id} name={currentPodcast?.name} author={currentPodcast?.artist} image={currentPodcast?.image} description={currentPodcast?.description}></PodcastCard>}
      
      {/* TODO: move this to its own component */}
      {chapters &&
        <><h1>Episodes: {chapters.length}</h1><table>
          <thead>
            <tr>
              <th>Title</th>
              <th>Date</th>
          ...
          <tbody>
            {chapters.map(chapter => (
              <tr key={chapter.id}>
                <td><a href="#">{chapter.title}</a></td>
                <td>{formatDate(chapter.publishDate)}</td>
